# (c) 2026 Kanin Raksaraj (P'Ice). All Rights Reserved.
...
Patent Claim 14
---------------
"A hierarchical Bayesian architecture for multimodal fusion that separates
population prior from individual posterior, applies Golden Ratio structural
constraints at each level, and supports federated learning."
...
Two-level hierarchy:
    ...
    Level 0 (Population) - HyperPrior
        Beta(alpha_pop, beta_pop) shared across organisations/users.
        alpha_pop / beta_pop ratio is constrained to phi at initialization.
        Updated via federated aggregation (no raw data exchange).

    Level 1 (Individual) - LocalPosterior (one per user/org)
        Beta(alpha_ind, beta_ind) starts from population prior,
        then updates on local observations only.
        Individual posterior is the engine inference result.

Golden Ratio Structural Constraint
----------------------------------
At every level the ratio alpha/beta is initialized to phi:

    Population init: alpha_pop = phi * s_pop, beta_pop = s_pop
    Individual init: alpha_ind = phi * s_ind, beta_ind = s_ind
                     where s_ind = (alpha_pop + beta_pop) / 2

Federated Learning Protocol
---------------------------
NamoNexus uses a data-minimized aggregation approach:

1. Each local model serializes (alpha_ind, beta_ind) sufficient statistics only.
2. The aggregator computes a phi-weighted average of individual statistics.
3. The aggregate updates the population-level HyperPrior.
4. Local models can warm-start from the new HyperPrior without sharing raw observations.
...
_PHI: float       = (1.0 + math.sqrt(5.0)) / 2.0   # approx 1.6180
_PHI_RECIP: float = 1.0 / _PHI                      # approx 0.6180
_PHI_SQ: float    = _PHI ** 2                       # approx 2.6180
...
class HierarchicalConfig:
    ...
    individual_scale_multiplier:
        ...
        Default = _PHI_RECIP approx 0.618 (individual prior is softer than population).
    phi_rank_decay:
        Base for phi-rank decay in federated aggregation.
    ...
    dp_noise_std:
        Gaussian noise std-dev added to (alpha, beta) before sharing for
...
class PopulationModel:
    """
    Population-level hyper-prior: a Beta(alpha_pop, beta_pop) shared across all clients.

    Golden Ratio structural constraint: alpha_pop / beta_pop is initialised to phi.
    ...
    def concentration(self) -> float:
        """Total concentration (alpha + beta) - proxy for strength of population belief."""
        return self._alpha + self._beta

    @property
    def phi_ratio(self) -> float:
        """Current alpha/beta ratio (should be close to phi when well-calibrated)."""
    ...
    def _federated_update(self, new_alpha: float, new_beta: float) -> None:
        ...
        if new_alpha <= 0 or new_beta <= 0:
            logger.warning(
                ...
                "(alpha=%.4f, beta=%.4f) - skipped", new_alpha, new_beta
            )
            return
        ...
        self._agg_count += 1
        logger.info(
            "PopulationModel federated update #%d | alpha=%.4f beta=%.4f ratio=%.4f",
...
class LocalModel:
    """
    Individual-level posterior: Beta(alpha_ind, beta_ind), cold-started from population.
    ...
    ----------
    alpha_init:
        Initial alpha drawn from the population prior.
    beta_init:
        Initial beta drawn from the population prior.
    client_id:
    ...
    def from_population(
        ...
        The individual scale is set to:
            s_ind = (alpha_pop + beta_pop) * individual_scale_multiplier / 2

        The Golden Ratio constraint is preserved:
            alpha_ind = phi * s_ind,  beta_ind = s_ind  (ratio = phi exactly at cold-start)
        ...
        # alpha / (alpha + beta) = pop_mean  and  alpha + beta = s_ind  -> alpha = pop_mean * s_ind
        alpha_init = pop_mean * s_ind
        beta_init  = (1.0 - pop_mean) * s_ind

        # Guarantee minimum concentration = 2 (1 alpha, 1 beta) so posterior is defined
        ...
        logger.debug(
            "LocalModel cold-start | id=%s alpha=%.4f beta=%.4f from population mean=%.4f",
    ...
    def update(
        ...
        ----------
        score:
            Modality-level score in [0, 1].
        confidence:
            Reliability in [0, 1] - scales effective observation count.
        modality:
    ...
    @property
    def phi_ratio(self) -> float:
        """Current alpha/beta (should drift toward pop mean, not necessarily phi)."""
        return self._alpha / self._beta if self._beta > 0 else float("inf")
    ...
    def sufficient_stats(self, apply_dp: bool = True) -> Tuple[float, float]:
        """
        Return (alpha_local, beta_local) - the only data shared in federated aggregation.
...
class FederatedAggregator:
    """
    Coordinates phi-weighted federated aggregation of local model statistics.

    Design Principles
    -----------------
    * **No raw data** - only (alpha_k, beta_k) sufficient statistics are shared.
    * **phi-rank weighting** - clients with more data get exponentially more
      influence via phi^(rank_k - 1) weighting (Patent Claim 14).
    ...
    * **Convergence tracking** - ``convergence_history`` records how
    ...
    # ------------------------------------------------------------------
    # phi-weighted aggregation (core of Patent Claim 14)
    # ------------------------------------------------------------------
    ...
    def aggregate(self, apply_dp: bool = True) -> Dict[str, Any]:
        ...
        Steps
        -----
        1. Collect (alpha_k, beta_k) from each eligible client (obs >= min threshold).
        2. Rank clients by observation count (most data -> rank 1).
        3. Compute phi-rank weights: w_k = phi^(1 - rank_k), normalised.
        4. Compute weighted aggregate: alpha_new = sum w_k alpha_k, beta_new = sum w_k beta_k.
        ...
        -------
        dict
            Aggregation round summary including phi-weights and new population params.
        """
        eligible = [
    ...
    def convergence_summary(self) -> Dict[str, Any]:
        ...
        Returns delta (|alpha_new - alpha_old| + |beta_new - beta_old|) per round,
